...
from .models import UserProfile, Medicine, MedicineCategory, Supplier, StockMovement, Dispensing, DispensingItem
import json
...
def dashboard_view(request):
    ...
    stock_val_qs      = Medicine.objects.filter(is_active=True).annotate(
                            line_value=ExpressionWrapper(
                                F('selling_price') * F('stock_quantity'),
                                output_field=DecimalField()
                            )
                        ).aggregate(total=Sum('line_value'))
    total_stock_value = stock_val_qs['total'] or 0
    ...
    low_stock_items = Medicine.objects.filter(
                          is_active=True, stock_quantity__gt=0,
                          stock_quantity__lte=F('reorder_level')).order_by('stock_quantity')[:8]
    expiring_items  = Medicine.objects.filter(
                          is_active=True,
                          expiry_date__lte=expiry_threshold,
                          expiry_date__gte=today).order_by('expiry_date')[:6]
...
def inventory_view(request):
    ...
    medicines_qs = Medicine.objects.select_related('category', 'supplier').filter(is_active=True)
    ...
    if status_filter == 'low_stock':
        medicines_qs = medicines_qs.filter(stock_quantity__gt=0, stock_quantity__lte=F('reorder_level'))
    elif status_filter == 'out_of_stock':
        medicines_qs = medicines_qs.filter(stock_quantity=0)
    elif status_filter == 'expiring':
        medicines_qs = medicines_qs.filter(expiry_date__lte=expiry_threshold, expiry_date__gte=today)
    elif status_filter == 'expired':
        medicines_qs = medicines_qs.filter(expiry_date__lt=today)

    if highlight_pk:
        try:
            highlighted  = medicines_qs.filter(pk=highlight_pk)
            rest         = medicines_qs.exclude(pk=highlight_pk)
            medicines_qs = list(highlighted) + list(rest)
        except Exception:
            pass
    ...
    context = {
        ...
        'low_stock_items': Medicine.objects.filter(
                               is_active=True, stock_quantity__gt=0,
                               stock_quantity__lte=F('reorder_level')).order_by('stock_quantity')[:10],
        'expiring_items':  Medicine.objects.filter(
                               is_active=True,
                               expiry_date__lte=expiry_threshold,
                               expiry_date__gte=today).order_by('expiry_date')[:10],
        'out_of_stock_items': Medicine.objects.filter(
                               is_active=True, stock_quantity=0).order_by('medicine_name')[:10],
        'total_medicines':     Medicine.objects.filter(is_active=True).count(),
        'low_stock_count':     Medicine.objects.filter(
                                   is_active=True, stock_quantity__gt=0,
                                   stock_quantity__lte=F('reorder_level')).count(),
        ...
        try:
            ...
            if medicine.stock_quantity > 0:
                StockMovement.objects.create(
                    medicine=medicine, movement_type='in',
                    quantity=medicine.stock_quantity,
                    quantity_before=0, quantity_after=medicine.stock_quantity,
...
def inventory_edit_view(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            medicine.purchase_price   = request.POST.get('purchase_price') or 0
            ...
            if old_qty != new_qty:
                StockMovement.objects.create(
                    medicine=medicine, movement_type='adjust',
                    quantity=new_qty - old_qty,
                    quantity_before=old_qty, quantity_after=new_qty,
                    notes=f'Manual edit adjustment ({old_qty} → {new_qty})',
...
def inventory_stock_adjust(request, pk):
    if request.method == 'POST':
        try:
            ...
            medicine.save()
            StockMovement.objects.create(
                medicine=medicine, movement_type=move_type,
                quantity=actual_qty,
...
def stock_management_view(request):
    ...
    movements_qs = StockMovement.objects.select_related(
        'medicine', 'performed_by', 'supplier').all()
...
def stock_in_view(request):
    if request.method == 'POST':
        try:
            ...
            update_batch   = request.POST.get('update_batch') == 'on'
            ...
            if update_batch:
                if batch_number:   medicine.batch_number  = batch_number
                if expiry_date:    medicine.expiry_date   = expiry_date
                if supplier_id:    medicine.supplier_id   = supplier_id
                if purchase_price: medicine.purchase_price = purchase_price

            medicine.save()
            StockMovement.objects.create(
                medicine=medicine, movement_type='in',
                quantity=qty, quantity_before=old_qty, quantity_after=medicine.stock_quantity,
                supplier_id=supplier_id, batch_number=batch_number, expiry_date=expiry_date,
                purchase_price=purchase_price, reference_no=reference_no,
                notes=notes, performed_by=request.user,
            )
            messages.success(request,
                f'Stock In: +{qty} units of "{medicine.medicine_name}". '
                f'New stock: {medicine.stock_quantity}.')
        except Exception as e:
    ...
    if request.method == 'POST':
        try:
            ...
            StockMovement.objects.create(
                medicine=medicine, movement_type=move_type,
                quantity=-qty, quantity_before=old_qty, quantity_after=medicine.stock_quantity,
                dispensed_to=dispensed_to, prescription_no=prescription_no,
                reference_no=reference_no, notes=notes, performed_by=request.user,
            )
...
def stock_adjust_view(request):
    if request.method == 'POST':
        try:
            ...
            StockMovement.objects.create(
                medicine=medicine, movement_type=move_type,
                quantity=diff, quantity_before=old_qty, quantity_after=new_qty,
                reference_no=reference_no, notes=notes, performed_by=request.user,
            )
            direction = f'+{diff}' if diff >= 0 else str(diff)
            messages.success(request,
                f'Adjustment: "{medicine.medicine_name}" {old_qty} → {new_qty} ({direction}).')
        except Exception as e:
...
def stock_medicine_info(request):
    ...
    if not pk:
        return JsonResponse({'found': False})
    try:
        m = Medicine.objects.get(pk=pk, is_active=True)
        return JsonResponse({
            'found':          True,
            'medicine_name':  m.medicine_name,
            'generic_name':   m.generic_name,
...
def supplier_detail_view(request, pk):
    """AJAX — returns supplier detail + medicines + purchase history as JSON."""
    supplier = get_object_or_404(Supplier, pk=pk)

    medicines = list(
        supplier.medicines.filter(is_active=True)
...
def reports_view(request):
    ...
    expired_count        = Medicine.objects.filter(
                               is_active=True, expiry_date__lt=today).count()
    expiring_soon_count  = Medicine.objects.filter(
                               is_active=True,
                               expiry_date__gte=today,
                               expiry_date__lte=expiry_threshold).count()
 
    stock_value = Medicine.objects.filter(is_active=True).annotate(
    ...
    current_stock_qs = Medicine.objects.filter(is_active=True).select_related(
        'category', 'supplier'
    ...
    # Expired
    expired_qs = Medicine.objects.filter(
    ...
    expiring_qs = Medicine.objects.filter(
        is_active=True, expiry_date__gte=today, expiry_date__lte=expiry_threshold
    ).select_related('category', 'supplier').order_by('expiry_date')
    ...
    cat_labels = json.dumps([x['category__name'] or 'Uncategorized' for x in category_stock])
    ...
    # Revenue from sales (selling_price × dispensed qty)
    ...
    # ── FINANCIAL TAB DATA ────────────────────────────────────────────────────
    # Cost of goods sold (purchase_price × dispensed qty from stock movements with price)
    ...
    # Stock in (purchases) cost per month
    monthly_cost = list(
        StockMovement.objects.filter(
            movement_type='in', purchase_price__isnull=False
        ).annotate(
            month=TruncMonth('created_at'),
            cost=ExpressionWrapper(
                F('quantity') * F('purchase_price'),
    ...
    profit_by_medicine = list(
        StockMovement.objects.filter(
            ...
            created_at__date__gte=date_from,
            created_at__date__lte=date_to,
        ).values('medicine__medicine_name', 'medicine__strength', 'medicine__purchase_price')
        .annotate(
    ...
    context = {
        ...
        'current_stock_qs':    current_stock_qs,
        'low_stock_qs':        low_stock_qs,
        'expired_qs':          expired_qs,
        'expiring_qs':         expiring_qs,
        ...
        'top_med_labels':    top_med_labels,
        'top_med_units':     top_med_units,
        'top_med_revenues':  top_med_revenues,
        ...
        'monthly_rev_labels':   monthly_rev_labels,
        'monthly_rev_values':   monthly_rev_values,
...
def report_export_excel(request):
    ...
    def style_data_row(row_num, col_count, alternate=False):
        fill_color = 'F8FAFC' if alternate else 'FFFFFF'
        alt_fill   = PatternFill('solid', fgColor=fill_color)
        for c in range(1, col_count + 1):
            cell           = ws.cell(row=row_num, column=c)
            cell.fill      = alt_fill
            cell.border    = cell_border
    ...
    if report_type == 'current_stock':
        ...
        qs = Medicine.objects.filter(is_active=True).select_related(
            'category', 'supplier').order_by('medicine_name')
        ...
        ws.title  = 'Low Stock'
        start_row = write_report_title('Low Stock Report',
                                       f'Medicines at or below reorder level — {today}')
        headers   = ['#', 'Medicine Name', 'Generic Name', 'Category',
        ...
        ws.title  = 'Expired Medicines'
        start_row = write_report_title('Expired Medicines Report', f'As of {today}')
        headers   = ['#', 'Medicine Name', 'Generic Name', 'Category',
                     'Strength', 'Batch Number', 'Expiry Date', 'Stock Qty', 'Supplier']
        ws.append([])
        for col, h in enumerate(headers, 1):
            ws.cell(row=start_row, column=col).value = h
        style_header_row(start_row, len(headers))
        ...
        for i, m in enumerate(qs, 1):
            ...
            ws.append([i, m.medicine_name, m.generic_name,
                        m.category.name if m.category else '—',
                        m.strength, m.batch_number or '—',
                        str(m.expiry_date) if m.expiry_date else '—',
                        m.stock_quantity,
                        m.supplier.name if m.supplier else '—'])
            style_data_row(r, len(headers), i % 2 == 0)
            # Highlight expiry date red
        ...
        start_row = write_report_title('Expiring Soon Report',
                                       f'Medicines expiring within 90 days — {today}')
        headers   = ['#', 'Medicine Name', 'Generic Name', 'Category',
                     'Strength', 'Batch Number', 'Expiry Date', 'Days Left', 'Stock Qty', 'Supplier']
        ws.append([])
        for col, h in enumerate(headers, 1):
            ws.cell(row=start_row, column=col).value = h
        style_header_row(start_row, len(headers))
        ...
        for i, m in enumerate(qs, 1):
            r        = start_row + i
            days_left = (m.expiry_date - today).days if m.expiry_date else 0
            ws.append([i, m.medicine_name, m.generic_name,
                        m.category.name if m.category else '—',
                        m.strength, m.batch_number or '—',
                        str(m.expiry_date) if m.expiry_date else '—',
                        days_left, m.stock_quantity,
                        m.supplier.name if m.supplier else '—'])
            style_data_row(r, len(headers), i % 2 == 0)
            days_cell = ws.cell(row=r, column=8)
            days_cell.font = Font(bold=True, color='D97706' if days_left <= 30 else '1A2433')
 
        col_widths = [5, 28, 25, 18, 12, 16, 14, 10, 10, 22]
 
    elif report_type == 'sales':
        ws.title  = 'Sales Report'
        start_row = write_report_title('Sales Report',
                                       f'{date_from} to {date_to}')
        headers   = ['#', 'Date', 'Medicine', 'Strength', 'Qty Dispensed',
                     'Selling Price', 'Revenue', 'Dispensed To',
                     'Prescription No.', 'Performed By']
        ws.append([])
        for col, h in enumerate(headers, 1):
            ws.cell(row=start_row, column=col).value = h
        style_header_row(start_row, len(headers))
        ...
        for i, mv in enumerate(qs, 1):
            ...
            ws.append([
                ...
                mv.medicine.medicine_name,
                mv.medicine.strength,
                qty,
                float(mv.medicine.selling_price),
                float(rev),
                mv.dispensed_to or '—',
                mv.prescription_no or '—',
                by,
            ])
            style_data_row(r, len(headers), i % 2 == 0)
        ...
        start_row = write_report_title('Financial Report',
                                       f'{date_from} to {date_to}')
        headers   = ['#', 'Medicine', 'Strength', 'Units Sold',
                     'Selling Price', 'Purchase Price', 'Revenue',
        ...
        for i, row in enumerate(qs, 1):
            ...
            ws.append([
                i,
                row['medicine__medicine_name'],
                row['medicine__strength'],
                row['units'],
                float(row['medicine__selling_price']),
            ...
            profit_cell = ws.cell(row=r, column=9)
            profit_cell.font = Font(
                bold=True,
...
def report_export_pdf(request):
    ...
    elements.append(Paragraph(
        report_titles.get(report_type, 'Report'), title_style
    ))
    elements.append(Paragraph(
    ...
    if report_type == 'current_stock':
        ...
        for i, m in enumerate(qs, 1):
            ...
            ts.add('TEXTCOLOR', (8, row_idx), (8, row_idx), color)
            ts.add('FONTNAME',  (8, row_idx), (8, row_idx), 'Helvetica-Bold')
        col_widths = [1*cm, 5.5*cm, 4.5*cm, 3*cm, 2.5*cm, 2.5*cm,
                      1.8*cm, 2*cm, 1.8*cm]
        ...
        for i, m in enumerate(qs, 1):
            data.append([i, m.medicine_name, m.generic_name,
                          ...
                          m.reorder_level - m.stock_quantity,
                          m.supplier.name if m.supplier else '—'])
        col_widths = [1*cm, 5.5*cm, 4.5*cm, 3*cm, 2.5*cm,
                      1.8*cm, 2*cm, 2*cm, 4*cm]
        ...
        data = [['#', 'Medicine', 'Generic', 'Category',
                 'Strength', 'Batch', 'Expiry Date', 'Stock', 'Supplier']]
        ts   = make_table_style(qs.count() + 1, 9)
        for i, m in enumerate(qs, 1):
            data.append([i, m.medicine_name, m.generic_name,
                          m.category.name if m.category else '—',
        ...
        col_widths = [1*cm, 5.5*cm, 4.5*cm, 2.5*cm, 2.5*cm,
                      2.5*cm, 2.8*cm, 1.5*cm, 4*cm]
        ...
        data = [['#', 'Medicine', 'Generic', 'Category',
                 'Strength', 'Batch', 'Expiry Date', 'Days Left', 'Stock']]
        ts   = make_table_style(qs.count() + 1, 9)
        for i, m in enumerate(qs, 1):
            days_left = (m.expiry_date - today).days if m.expiry_date else 0
            data.append([i, m.medicine_name, m.generic_name,
        ...
        for i, mv in enumerate(qs, 1):
            ...
            data.append([
                i,
                mv.created_at.strftime('%m/%d/%Y'),
                mv.medicine.medicine_name,
                mv.medicine.strength,
                qty,
                f'₱{mv.medicine.selling_price:,.2f}',
                f'₱{rev:,.2f}',
                mv.dispensed_to or '—',
                by,
            ])
        ...
        col_widths = [1*cm, 2.5*cm, 5.5*cm, 2.5*cm, 1.5*cm,
                      2.5*cm, 2.5*cm, 4*cm, 3.5*cm]
        ...
        for i, row in enumerate(qs, 1):
            ...
            ts.add('FONTNAME',  (6, i), (6, i), 'Helvetica-Bold')
        col_widths = [1*cm, 6*cm, 2.5*cm, 2*cm,
                      3*cm, 3*cm, 3.5*cm, 2.5*cm]
    else:
        data       = [['No data']]
...
# DISPENSING
# ─────────────────────────────────────────────────────────────────────────────
# Add this import at the top of views.py alongside existing imports:
# from .models import UserProfile, Medicine, MedicineCategory, Supplier, StockMovement, Dispensing, DispensingItem

@login_required(login_url='login')
@user_passes_test(is_staff_or_admin, login_url='dashboard')
def dispensing_view(request):
    """Main dispensing / POS page."""
    today = timezone.now().date()
    ...
    units_today = DispensingItem.objects.filter(
                      ...
                      t=Sum('quantity'))['t'] or 0
    revenue_today = today_txns.aggregate(
                        t=Sum('total_amount'))['t'] or 0

    month_start = today.replace(day=1)
    txn_month   = Dispensing.objects.filter(
...
def dispensing_create(request):
    """
    AJAX POST — process a dispensing cart.
    Creates Dispensing + DispensingItem records and deducts stock via StockMovement.
    ...
    try:
        ...
        pharmacist_name = request.user.get_full_name() or request.user.username
...
def dispensing_receipt(request, pk):
    """AJAX GET — return receipt data for an existing dispensing transaction."""
...
# Append these two views to your existing views.py
# ─────────────────────────────────────────────────────────────────────────────
 
@login_required(login_url='login')
def category_search_ajax(request):
    """
    GET  ?q=<term>  — returns matching categories as JSON.
    Returns up to 20 results. If q is empty, returns all categories.
    """
    q  = request.GET.get('q', '').strip()
    ...
    return JsonResponse({
        'results': [{'pk': c.pk, 'name': c.name} for c in qs]
    })
...
def category_create_ajax(request):
    """
    POST  {name: "..."}  — creates a new category if the name is unique.
    Returns the new category's pk and name as JSON.
    """
    if request.method != 'POST':
        return JsonResponse({'error': 'Invalid method.'}, status=405)
